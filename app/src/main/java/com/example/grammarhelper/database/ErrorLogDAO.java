...
public class ErrorLogDAO {
    ...
    public long insertErrorLog(GrammarError error) {
        ...
        return database.insert(DatabaseHelper.TABLE_ERROR_LOG, null, values);
    }
    ...
    public List<GrammarError> getErrorsBySession(int sessionId) {
        ...
        Cursor cursor = database.query(DatabaseHelper.TABLE_ERROR_LOG, null, selection, selectionArgs, null, null, null);
    ...
    public void clearErrorLog() {
        database.delete(DatabaseHelper.TABLE_ERROR_LOG, null, null);
    }

    public int getTotalFixedCount() {
        Cursor cursor = database.rawQuery("SELECT COUNT(*) FROM " + DatabaseHelper.TABLE_ERROR_LOG +
